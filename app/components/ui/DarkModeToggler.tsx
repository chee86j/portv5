...
export function DarkModeToggler({ className = "" }: DarkModeTogglerProps) {
  const { resolvedTheme, setTheme } = useTheme();
  /* We only want to render the control after hydration.
     `useSyncExternalStore` introduced in React 18 gives us a server snapshot (`false`) and a client
     snapshot (`true`) without using an effect that immediately sets state.
     That avoids the `react-hooks/set-state-in-effect` lint error that the
     legacy mounted-state pattern triggers for this exact use case.
  */
  ...
  /* Legacy method below for reference:
  ...
  */
  ...
  return (
    <div className={`toggle-cont ${className}`.trim()}>
      <input
        type="checkbox"
        id={toggleId}
        checked={isDark}
